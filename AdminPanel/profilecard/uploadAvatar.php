...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['avatar']) && isset($_POST['EmailAdmin'])) {

    // Retrieve the uploaded file and EmailAdmin from the POST data
    ...
    if (move_uploaded_file($avatar['tmp_name'], $targetFilePath)) {
        ...
        if ($usersData && isset($usersData['users'])) {
            // Find the user by EmailAdmin and update their avatar field
